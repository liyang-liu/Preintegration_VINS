function [J_mat] = JObject2Matrix( J_obj )
    ...
    %% dUv_dX
    numUV = size( J_obj.dUv_dX, 1 );
    for i=1:numUV
        if ( ~isempty( J_obj.dUv_dX[i].dAbgxyz ) )
            %dAbgxyz
            J_row = [ J_row; J_obj.dUv_dX[i].dAbgxyz.row(:) ];
            J_col = [ J_col; J_obj.dUv_dX[i].dAbgxyz.col(:) ];
            J_vec = [ J_vec; J_obj.dUv_dX[i].dAbgxyz.val(:) ];        
        end
    ...
    %% dIntlDelta_dX
    numIntlDelta = size( J_obj.dIntlDelta_dX, 1 );
    for i=1:numIntlDelta
        ...
        if ( ~isempty( J_obj.dIntlDelta_dX[i].dDv_dX.dDv_dAbg_1 ) )
            %dDphi_dAbg_1
            J_row = [ J_row; J_obj.dIntlDelta_dX[i].dDphi_dX.dDv_dAbg_1.row(:) ];
            J_col = [ J_col; J_obj.dIntlDelta_dX[i].dDphi_dX.dDv_dAbg_1.col(:) ];
            J_vec = [ J_vec; J_obj.dIntlDelta_dX[i].dDphi_dX.dDv_dAbg_1.val(:) ];            
        end

        %dDphi_dAbg_2
        J_row = [ J_row; J_obj.dIntlDelta_dX[i].dDphi_dX.dDv_dAbg_2.row(:) ];
        J_col = [ J_col; J_obj.dIntlDelta_dX[i].dDphi_dX.dDv_dAbg_2.col(:) ];
        J_vec = [ J_vec; J_obj.dIntlDelta_dX[i].dDphi_dX.dDv_dAbg_2.val(:) ];            
    ...
    J_mat = sparse( J_row, J_col, J_vec );
