...
nAllposes = 12;%13;%4;%20;%10;%5;%240;%60;% 350;%
nPoseOld = 1;
nAddPoses = 1;%5;%50;%30;%1;%10;%
nPoseNew = 4;%5;%15;%30;%80;%50;%120;%30;%25;%20;%18;%13;%3;%5;%10;%4;%50;%9;%2;%170;%6;%120;%
...
kfspan = 10;%5;%20;%1;%15;%50;%30;%2;% Choose keyframes
kfids = 1:kfspan:1200;
nMinObsTimes = 2;%5;%50;%30;%20;%3;%4;%6;%
fMaxDistance = 50;%1e6;%1e2;%5e2;%80;%30;%20;%10;%84;%60;
...
if(bSimData == 1)
    ...
    sigma_w_real = 0.03;% 0;%0.01;%
    sigma_f_real = 0.2;%0;%0.1;%0.    
    sigma_w_cov = 2*sigma_w_real;%;%1e-3; 
    sigma_f_cov = 2*sigma_f_real;%1;%1e-3;%0.0775; % Siga for the noises to be added to the IMU raw data (wi,ai)
    sigma_uov_real = 1e-2;%1e-3;%1;% 
    sigma_uov_cov = 5*sigma_uov_real;%1;%
    sigma_d_real = 0;%
    sigma_d_cov = 0.1;
    sigma_bf_real = 0;% 1e-3;
    sigma_bw_real = 0;% 1e-3;
    sigma_g_cov = 1e-4;
	sigma_au2c_cov = 1e-4;
    sigma_tu2c_cov = 1e-4;
    sigma_bf_cov = 1e-4;
    ...
    imgdir = [];
    ImuTimestamps = 1:(nIMUrate):((nAllposes-1)*nIMUrate+1);
    nUV = nPts*2*nPoseNew;
    nIMUdata = (nPoseNew-1)*nIMUrate;
    dtIMU = ones(nAllposes, 1);
...
elseif(bMalaga == 1)
    ...
    sigma_w_cov = 1;%2*sigma_w_real;%5e1*(5e-2)*pi/180;%5e-1;%1;%1e-2;%1;%1e-2;%5e-1;%4.5e-3;%2.5e-1;%2e-1;%3.5e-1;%5e-1;%5e-2;%
    sigma_f_cov = 1;%*sigma_f_real;%5e1*2e-3;%1e-2;%1e-5;%1;%1e0;%4.5e-3;%2.5e-1;%2e-1;%3.5e-1;%1e0;%2e-3;%
    sigma_uov_real = 1e-3;%1;%1e-6;%1e-4;%
    sigma_uov_cov = 1;%2*sigma_uov_real;%1e-3;%
    sigma_g_cov = 1e-1;
	sigma_au2c_cov = 1e-1;    
	sigma_tu2c_cov = 1e-1; 
    sigma_bf_cov=1*2e-2;
...
elseif(bDinuka == 1)
    ...
    nIMUrate = 2e2;    
    sigma_w_real = 0.03;%0.0;%0.01;%0.15;% 5e-1;%1;%1e-2;%1;%1e-2;%5e-1;%4.5e-3;%2.5e-1;%2e-1;%3.5e-1;%5e-1;%5e-2;%
    sigma_f_real = 0.2;%0;%0.01;%0.1;%1;%  
    ...
    sigma_uov_real = 0.2;%0.5;%0;% 1e-2;%1e-3;%2e-3;%1e-1;%1;%1e-6;%1e-4;%
    sigma_uov_cov = 3*sigma_uov_real;%1;%1e-1;%1e-2;%
...
%     end
    ...
    idr = nPoseNew*nPts*2;% total number of camera observations: (ui,vi) 
    
    if(bPreInt == 0)
        nlenpp = nIMUrate*3*3; % IMU raw data observations between two key frames: (wi,ai,dTi)
        Zobs = zeros(idr+(nPoseNew-1)*nlenpp, 1);
    else
        Zobs = zeros(idr+(nPoseNew-1)*9, 1); % Number of observations for pre-integration: (ui,vi) + (dpi,dvi,dphii) 
    end
...
%     g0 = [0; 0; -9.8]; % g value in the first key frame
    ...
    %         fnSimIMUnCameraFeatures1Pts(arPts, nPts, Ru2c, Tu2c, g0, bf0, bw0, nIMUrate, bPreInt);%
        [imuData_cell, uvd_cell, Ru_cell, Tu_cell, Ru2c, Tu2c, vu] = ...
            fnSimIMUnFeaturesAtNPoses_helix(nAllposes, nPts, Ru2c, Tu2c, g0, bf0, bw0, nIMUrate, bPreInt);% line   


        if(bTestIMU == 1)
            fnTestIMU_cmp(nPoseNew, imuData_cell, Ru_cell, Tu_cell, ...
    ...
    %             uvd_cell{pid} = uvd_cell{pid} + gns;  
                [gns] = fnGenGaussNoise(2, nc, sigma_uov_real);
                ...
                [gns] = fnGenGaussNoise(1, nc, sigma_d_real);
                uvd_cell{pid}(3,:) = uvd_cell{pid}(3,:) + gns;   
    %             RptFeatureObs{pid} = FeatureObs;
            end
            RptFeatureObs = FeatureObs;
            RptFidSet = FeatureObs(:,1);
            % Add noise to imu data
            [gns] = fnGenGaussNoise(3, 1, sigma_bf_real);
            bf0 = bf0+gns;%fbiascef*sigmaf;
            [gns] = fnGenGaussNoise(3, 1, sigma_bw_real);
    ...
    %                 repmat([sigmaw*ones(1,3),sigmaf*ones(1,3)], nIMUrate,1) .* randn(size(imuData_cell{pid}.samples(:, 2:7)))/fZnoisescale;
                [gns] = fnGenGaussNoise(nr, nc, sigma_w_real);
                imuData_cell{pid}.samples(:, 2:4) = ...
                    imuData_cell{pid}.samples(:, 2:4) + gns;
                [gns] = fnGenGaussNoise(nr, nc, sigma_f_real);
    ...
    for pid=2:nAllposes%1e-2,1e-3+3*sigmaf+3*sigmaw
        [dp(:,pid), dv(:,pid), dphi(:,pid), Jd{pid}, Rd{pid}] = ...
            ...
            imuData_cell{pid}.samples, sigma_w_cov, sigma_f_cov); 
    end 
    
    if((bInitPnF5VoU == 1) && (bIMUodo == 0))
        for pid=2:(nAllposes-1)
            % Pick out T corresponding to the current pose 
            Timu = Tu_cell{pid}(:,1) - Tu_cell{pid-1}(:,1);
            fscaleGT(pid) = norm(Timu);
        end
        % The final pose
        Timu = Tu_cell{nAllposes} - Tu_cell{nAllposes-1}(:,1);
        fscaleGT(nAllposes) = norm(Timu);
        [dRi, dTi] = fnGetAllRelativePoses5MatchedFeatures(...
            nAllposes, K, fscaleGT, RptFeatureObs);        
    end
...
%         end
        ...
        if(bPreInt == 0)
            x = zeros((nPoseNew-1)*nIMUrate*6+nPts*3+3*((nPoseNew-1)*nIMUrate+1)+15+6,1);% one additional 6 for convenience
        else
        ...
        [R0imu, T0imu, v0imu] = fnCaluRTV0(bUVonly, bPreInt, nPoseOld, ImuTimestamps, ...
                x_old, RptFidSet_old);
        
        if(bInitPnF5VoU == 1)
            if(bIMUodo == 0)
                [Rcam, Acam, Tcam, Feature3D] = fnGetPoses5MatchedFeatures_Inc(...
                    nPoseOld, nPoses, R0imu, T0imu, K, fscaleGT, Feature3D, RptFeatureObs, ...
                    Tu2c, Ru2c);     
                %% Obtain relative poses from visual odometry            
    %             [Rcam, Acam, Tcam, Feature3D] = fnGetPoses5MatchedFeatures(nPoseNew, nPts, ...
...
%                     else
%                         idend = 6*nIMUrate*(nPoseOld-1)+3*nPts;
%                     end
%                     idstart = idend + 1;
%                     idend = idend + 3;
...
%                 [Rcam, Acam, Tcam, vimu, Feature3D] = fnGetPoses5IMUdata_Inc(nPoseOld, ...
%                     nPoseNew, nPoses, R0imu, T0imu, v0imu, dtIMU, g0, dp, dv, dphi, ...
%                     K, Feature3D, bMalaga, RptFeatureObs, Tu2c, Ru2c);
                [Rcam, Acam, Tcam, vimu, Feature3D, RptFidSet, RptFeatureObs] = fnGetPoses5IMUdata_Inc(nPoseOld, ...
                                nPoseNew, nPoses, R0imu, T0imu, v0imu, dtIMU, g0, dp, dv, dphi, ...
    ...
    %% Combine old and new poses into x        
            ...
            if(bPreInt == 1)
                ...
                if(nPoseOld > 1)
                    idstart = idend + 1; 
                    idend = idend + 6*(nPoseOld-1)*nIMUrate; 
                    x(idstart:idend) = x_old(idstart:idend);
                end
                idstart  = idend + 1; 
                idend = 6*(nPoseNew - 1)*nIMUrate;  
                tv = fnLinearInterpPoses_Inc(nPoses, nPoseOld, ABGimu, Timu, ...
                    ImuTimestamps);
                x(idstart:idend) = tv(:);
            end        
        end        
        ...
        if(nPoseOld > 1)
            if(bPreInt == 1)
                ...
                idend_old = 6*(nPoseOld-1)*nIMUrate;
            end
            idstart_old = idend_old + 1;
            idend_old = idend_old + 3*nPts;    
            x(idstart:idend) = x_old(idstart_old:idend_old);
        elseif(bInitPnF5VoU == 1)
            fp_c1 =  (Feature3D(RptFidSet, 5:7))';% select the first group 
            fp_u1 = Ru2c' * fp_c1 + repmat(Tu2c, 1, size(fp_c1, 2));
            x(idstart:idend) = fp_u1(:);
        end
    %     if(bInitPnF5VoU == 1)
    ...
    %     end
        ...
        if(bInitPnF5VoU == 1)
    %         idx = idx+nPts*3;
    %        [x, ~] = fnInitVelocity(nPoseNew, bPreInt, x, idx, g0, bf0, dp, dv, ...
    %            dtIMU, imuData_cell, nIMUdata, nIMUrate, dt); 
            [tv] = fnInitVelocity_Inc(nPoseNew, nPoseOld, nPoses, nPts, bPreInt,...
                g0, bf0, dp, dv, dtIMU, imuData_cell, nIMUrate, dt, x);
            if(bPreInt == 1)
                if(nPoseOld > 1)
                    idstart = idend + 1;
                ...
                if(nPoseOld > 1)
                    idstart = idend + 1;
                    idend = idend + 3*nPoseOld*nIMUrate;
                    idstart_old = idend_old + 1;
                    idend_old = idend_old + 3*nPoseOld*nIMUrate+1;
                    x(idstart:idend) = x_old(idstart_old:idend_old);
                end
                idstart = idend + 1;
%                 if(nPoseOld == 1)
                idend = idend + 3*(nPoses*nIMUrate+1);
%                 else
%                     idend = idend + 3*(nPoses-1)*nIMUrate;
%                 end
                x(idstart:idend) = tv;            
            end
        end
        ...
        if(nPoseOld == 1)
            ...
            idstart = idend + 1;
            idstart_old = idend_old + 1;
            x(idstart:end) = x_old(idstart_old:end);
        end


    %% Compose Ground truth x
        [xg] = fnGetXgroundtruth(nPoseNew, nAllposes, nPts, bPreInt, Ru_cell, Tu_cell, ...
    ...
    % Show initial value
        ...
        if(bShowFnP == 1)
            fnShowFeaturesnPoses(xg, nPoseNew, nPts, nIMUrate, bPreInt, 'Ground Truth Values');
            fnShowFeaturesnPoses(x, nPoseNew, nPts, nIMUrate, bPreInt, 'Initial Values');
        end    
    ...
    %     end   
        ...
        for(fid=1:nPts)% local id
            ...
            for(oid=1:nObs)
                if(RptFeatureObs(fid, (oid*3)) > nPoseNew)
                    break;
                end
    ...
    %     end  
        ...
        if(bPreInt == 0) % Non-pre-integration: put raw data  
            ...
            if(bPerfectIMUdlt == 0)
                ...
                dt = 1;
                Zobs((idr+1):(idr+9*(nPoseNew-1)),1) = fnCalPerfectIMUdlt(x, nPoseNew, nPts, Jd, dt, bf0, bw0); 
            end
    ...
    if(bPreInt == 1)
        utid = idr+(nPoseNew-1)*3*3+15;% initialized with possible maximum size.
    else
        utid = idr + (nPoseNew - 1)*nlenpp+15;
    end
    ...
    tic
        if(bGNopt == 1)
        %% GN Iterations 
            [x] = fnVI_BA_general(nUV, K, x, nPoseNew, nPts, Jd, CovMatrixInv, ...
            ...
            [x,Reason,Info] = fnleastsquaresLM(nUV, K, x, nPoseNew, nPts, Jd, ...
    ...
    toc    
        ...
        if(bPreInt == 0)
            fprintf('\n###Poses[x(1-%d)], Features[x(%d-%d)], Velocity[x(%d-%d]###\n',...
                (nPoseNew-1)*nIMUrate*6, (nPoseNew-1)*nIMUrate*6+1, ...
                (nPoseNew-1)*nIMUrate*6+nPts*3, (nPoseNew-1)*nIMUrate*6+nPts*3+1, ...
                (nPoseNew-1)*nIMUrate*6+nPts*3+3*((nPoseNew-1)*nIMUrate+1));
        else
            fprintf('\n###Poses[x(1-%d)], Features[x(%d-%d)], Velocity[x(%d-%d)]###\n',...
    ...
    %     save('x_Jac.mat', 'xf');
        %% Show pose-feature graph
        if((nPoseOld == 1) || (nPoseNew == nAllposes))% bShowFnP == 1)
            fnShowFeaturesnPoses(xf, nPoseNew, nPts, nIMUrate, bPreInt, 'Final Values');
        end
        %% Show uncertainty
        if((nPoseOld == 1) || (nPoseNew == nAllposes))% bShowUncertainty == 1)
...
%             x = x(1:(size(xg,1)));
    ...
    %%%%%%%%%%%%    
        save('x_Jac.mat', 'x');
        %% Show pose-feature graph
        if(nPoseNew == nAllposes)%bShowFnP == 1)
        ...
        %% Show uncertainty
        if(nPoseNew == nAllposes)%bShowUncertainty == 1)
            fnCalnShowUncert_general(bUVonly, nUV, RptFeatureObs, ImuTimestamps, ...
                dtIMU, ef, K, x, nPoseNew, nPts, Jd, CovMatrixInv, nIMUrate, nIMUdata, bPreInt, ...
                bAddZg, bAddZau2c, bAddZtu2c, bAddZbf, bAddZbw, bVarBias);
        end
