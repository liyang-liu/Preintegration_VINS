function J = fnJdaw0_IMU_general(nUV, J, nPts, x, nIMUrate, nIMUdata, ...
    bAddZg, bAddZau2c, bAddZtu2c, bAddZbf, bAddZbw)%g, 
...
% J = sparse(3*N*nPoses+3*3*(nPoses-1), 3*N+6*(nPoses-1)+3*nPoses+5*3);
    idx_au2c = 6*(nIMUdata-1)+nPts*3+3*nIMUdata+3;%nIMUrate*(nPoses-1) ((nPoses-1)*nIMUrate+1)(nPoses-1)*6+nPts*3+3*nPoses+4;
    idx_g = 6*(nIMUdata)+nPts*3+3*(nIMUdata+1);%(nPoses-1)*6+nPts*3+3*nPoses+1;
    g = x((idx_g+1):(idx_g+3));
    idx_bf = 6*(nIMUdata)+nPts*3+3*(1+nIMUdata)+9;%;(nPoses-1)*6+nPts*3+3*nPoses+10;
%     bf = x(idx_bf:(idx_bf+2));
    idx_bw = 6*(nIMUdata)+nPts*3+3*(1+nIMUdata)+12;%(nPoses-1)*6+nPts*3+3*nPoses+13;
