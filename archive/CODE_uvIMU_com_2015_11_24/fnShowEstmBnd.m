function [] = fnShowEstmBnd(ef, Td, nPoses, nPts, nIMUrate, bPreInt)
%%
% Au,Tu,f,vu, 
...
if(bPreInt == 1)
    ...
    nMin = 6*(nPoses - 1)*nIMUrate+1;
    nMax = 6*(nPoses - 1)*nIMUrate + 3*nPts;
end
...
if(bPreInt == 1)
    ...
    nMin = 6*(nPoses - 1)*nIMUrate + 3*nPts+1;
    nMax = 6*(nPoses - 1)*nIMUrate + 3*nPts + 3*((nPoses - 1)*nIMUrate+1);
end
...
if(bPreInt == 1)
    ...
    nMin = 6*(nPoses - 1)*nIMUrate + 3*nPts + 3*((nPoses - 1)*nIMUrate+1)+1;
    nMax = 6*(nPoses - 1)*nIMUrate + 3*nPts + 3*((nPoses - 1)*nIMUrate+1)+15;
end
tv = ef(nMin:nMax);
tv = reshape(tv, 3, []);
tvb = Td(nMin:nMax);
