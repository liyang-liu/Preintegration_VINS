function [imuData_cell, uvd_cell, Ru_cell, Tu_cell, Ru2c, Tu2c, vu] = ...
            fnSimIMUnCameraFeatures1Pts(arPts, nPts, Ru2c, Tu2c, g, ...
            bf, bw, nIMUrate, bPreInt)
...
% Based on the specified poses, calculate possible rotation
% that enable the camera to face the features at all times.

t = 0:(1.0/nIMUrate):(1-(1.0/nIMUrate));
[px, vx, ax, pxpp] = fnInterpPoses(nPoses, arPts(:,1), t);
[py, vy, ay, pypp] = fnInterpPoses(nPoses, arPts(:,2), t);
[pz, vz, az, pypp] = fnInterpPoses(nPoses, arPts(:,3), t);
...
cfy = Tu2c(2) + 6; 
cfz = Tu2c(3);
%All of the features are spreading on  a ball.
...
radius = 5e-1;%0.5;%1;%3; % the dimension of the circles
np = nf1c * nc;
fp0 = zeros(3, np);
% uvd1 = fp1;
% fp2 = fp1;
...
% on nc circles	
	pids = (1:nf1c);
	fp0(2, pids) = cfy;%zcirc; % IMUy
	%uvd_cell{1}(3, pids) = zcirc;	
	fp0(1, pids) = cfx + radius * cos(fangle);% IMUx
	fp0(3, pids) = cfz + radius * sin(fangle);% IMUz
	%zcirc = zcirc - radius;%fdcirc2camera;
	fangle = fangle + deltangle; % rotate an angle

if (nc == 2)	
	pids = (1:nf1c) + ((nc - 1) * nf1c);
	fp0(1, pids) = cfx;%zcirc; % IMUy
	%uvd_cell{1}(3, pids) = zcirc;	
	fp0(2, pids) = cfy + radius * cos(fangle);% IMUx
	fp0(3, pids) = cfz + radius * sin(fangle);% IMUz
end
...
%     Tu_cell{cid} = Tu_cell{cid-1} + tT;% Already in the initial coordinates 
    ...
    dz = spn * 5e-3 + (5e-3 ^ cid);%spn*2e-1 ^ (cid-1);%2e-1 ^ (cid-1);%0.0;%-0.1;%
    tT = [dx, dy, dz]';
    % Rotation
...
%             dabg = dlta *[1,1,1]';
    ...
    if(cid < nPoses)
        ...
        [dataIMU] = fnSimuIMU(theta{cid}, dthetadt, dvdt, g, bf, bw);  
        x0(1:3, 1) = [alpha; beta; gamma];
...
if(bPreInt == 0)
    ...
    for cid=1:(nPoses-1)
        Tu_cell{cid} = p{cid};
        for(did=1:nIMUrate)
            Ru_cell{cid}{did} = fRfrmABG(theta{cid}(1,did), ...
