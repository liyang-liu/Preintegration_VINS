function [J] = fnJduvd_CnU_gq(nJacs, idRow, idCol, K, x, nPoses, nPts, nIMUdata, ...
    ImuTimestamps, RptFeatureObs, bUVonly, bPreInt, nUV, bAddZg, bAddZau2c, ...
...
%         Tc = Ru' * Tu2c + Tu;
    ...
    % 	[dxyz] = fxyz5abgxyz_dr(alpha, beta, gamma, p3d0, N);
        %%% dxyz/d((abgxyz)c,(abgxyz)u2c),3x6,3x6
...
%         tidstart = tidend + 1;
% %         tidend = tidend + 6;
%         %tmpv(tidstart:tidend) = (duvdfxyz(:))';
%         
%         %% d(uv)/d(abgxyz)u2c part
%         duvdxyz_u2c = duvd(1:2,:) * dxyz_u2c;
%         tidstart = tidend + 1;
%         tidend = tidend + 12;
%         tmpv(tidstart:tidend) = (duvdxyz_u2c(:))';
...
% J = sparse(idRow, idCol, tmpv);%%J(1:(nUV+3*2+1), :);
