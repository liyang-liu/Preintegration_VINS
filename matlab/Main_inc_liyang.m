...
%% The main switch
    ...
    elseif(InertialDelta_options.bDinuka == 1)
        ...
        [SLAM_Params.Au2c_true(1), SLAM_Params.Au2c_ture(2), SLAM_Params.Au2c_true(3) ] = ...
                                                fn_ABGFromR(SLAM_Params.Ru2c_true);
...
%% Incrementally construct x, z and cov, then solve them trhough iterations 
    while(nPoseOld < nAllposes)
        ...
        %% X---the state vector
        X_obj = SLAM_X_Define( nPts, nPoseNew, nIMUdata );
        Xg_obj = X_obj;
    ...
    ef = SLAM_X_Object2Vector( SLAM_X_ObjectDiff( X_obj, Xg_obj ));
    [maxe, idx] = max(abs(ef));
    ...
    if((nPoseOld == 1) || (nPoseNew == nAllposes))
        ...
        for(pid = 2:nPoseNew)
            ...
            Rimu = fn_RFromABG( Aimu(1), Aimu(2), Aimu(3));
