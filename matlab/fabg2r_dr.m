function [drda, drdb, drdg] = fabg2r_dr(alpha, beta, gamma)
...
drda = fRx_dr(alpha) * fRy(beta) * fRz(gamma);
drdb = fRx(alpha) * fRy_dr(beta) * fRz(gamma);
drdg = fRx(alpha) * fRy(beta) * fRz_dr(gamma);
