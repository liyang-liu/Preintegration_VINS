function [X, RptFidSet, RptFeatureObs, nPts] = fnCompXFromOdometry( ...
    ...
    elseif(nPoseOld > 1)
        ...
        for(id=1:size(pids,1))%2:pid_end)
            if(id > 1)
                pid = pids(id);
                idx = 6*(pid-2);
                %R0imu(:, :, id) = fnRFromABG(x_old(idx+1), x_old(idx+2), x_old(idx+3));
    ...
    % Intial values of poses and IMU states
     if(PreIntegration_options.bIMUodo == 1)
        ...
        [Rcam, ~, Tcam, Feature3D] = fnGetPosesFromMatchedFeatures(nPoseNew, nPts, ...
    ...
    if(PreIntegration_options.bUVonly == 0)
        ...
        [tv,xcol,~] = fnCalcVFromKposes_Inc( nPoseNew, nPoseOld, ...
